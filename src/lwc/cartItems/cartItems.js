...
import changeCartStatusToActive from '@salesforce/apex/B2BGetInfo.changeCartStatusToActive';
...
export default class Items extends NavigationMixin(LightningElement) {
    ...
    cancelCheckout(){
        ...
        changeCartStatusToActive({cartId:this.recordId});
    }
    ...
    set cartItems(items) {
        ...
        this._items = (items || []).map((item) => {
            ...
            // Set default value for conditionCode
            // newItem.conditionCode = '';
            // checkProductAddedIntoCart({productId:item.cartItem.productDetails.productId,cartId:item.cartItem.cartId}).then((conditionCodeResult) => {
            //     newItem.conditionCode = JSON.stringify(conditionCodeResult.Condition_Code__r.Name).replace(/"/g, "");
            //     console.log('Condition Code :'+JSON.stringify(conditionCodeResult.Condition_Code__r.Name).replace(/"/g, ""));
            // })
    ...
    get displayItems() {
       ...
       // console.log('Items :'+JSON.stringify(this._items));
    ...
    handleDeleteCartItem(clickEvt) {
        ...
        // Extra refresh code writen for internal comment by @deb
