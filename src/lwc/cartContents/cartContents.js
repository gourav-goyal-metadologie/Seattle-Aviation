...
import checkProductAddedIntoCart from '@salesforce/apex/B2BGetInfo.checkProductAddedIntoCart';
...
export default class CartContents extends NavigationMixin(LightningElement) {
    ...
    get labels() {
        return {
            ...
            CreatedDateDesc: 'Newest to Oldest',
            CreatedDateAsc: 'Oldest to Newest',
            NameAsc: 'Name - A to Z',
    ...
    @track productIdConditionCodeMap=[];
    ...
    getConditionCode(){
        checkProductAddedIntoCart({cartId:this.recordId}).then((conditionCodeResult) => {
            for(var key in conditionCodeResult){
                if(conditionCodeResult[key].Condition_Code__r){
                    if(conditionCodeResult[key].Condition_Code_Price_Type__c){
                    this.productIdConditionCodeMap.push({productId:conditionCodeResult[key].Product2Id, cCode:conditionCodeResult[key].Condition_Code_Price_Type__c});
                    }else{
                        this.productIdConditionCodeMap.push({productId:conditionCodeResult[key].Product2Id, cCode:conditionCodeResult[key].Condition_Code__r.Name});
                    }
                }else{
                    this.productIdConditionCodeMap.push({productId:conditionCodeResult[key].Product2Id, cCode:'-'});
                }
            }
            this.updateCartItems();
        })
        ...
        // Call the 'getCartItems' apex method imperatively
         getCartItems({
            ...
            sortParam: this.sortParam
                ...
                if(this.productIdConditionCodeMap){
                    for(var key in result.cartItems){
                        result.cartItems[key].cartItem.ConditionCode='';
                        for(var keys in this.productIdConditionCodeMap){
                            if(this.productIdConditionCodeMap[keys].productId===result.cartItems[key].cartItem.productDetails.productId){
                                this.cartItems[key].cartItem.ConditionCode=this.productIdConditionCodeMap[keys].cCode;
                            }
                        }
                    }
                }
